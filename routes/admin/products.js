...
router.get('/:id/edit', async (req, res) => {
  try {
    ...
    if (!product) {
      return res.status(404).render('errors/404', { title: 'Product Not Found', layout: 'layouts/admin' });
    }
...
router.post('/', [
  ...
  try {
    ...
    if (req.body.id) {
      // Update
      if (!errors.isEmpty()) {
        const product = await Product.findById(req.body.id).populate('mediaIds');
        const media = await Media.find().sort({ createdAt: -1 }).limit(50);
    ...
    res.redirect(req.body.id ? `/admin/products/${req.body.id}/edit` : '/admin/products/new');
  }
});
...
router.post('/:id/delete', async (req, res) => {
  try {
    ...
    req.session.flash = { type: 'success', message: 'Product deleted successfully' };
    res.redirect('/admin/products');
  } catch (error) {
