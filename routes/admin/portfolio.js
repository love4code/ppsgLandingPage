...
async function generateUniqueSlug (title, excludeId = null) {
  let baseSlug = slugify(title || 'post', { lower: true, strict: true })
  ...
  while (await PortfolioPost.exists(query)) {
...
router.get('/:id/edit', async (req, res) => {
  try {
    const post = await PortfolioPost.findById(req.params.id).populate(
      'mediaIds'
    )
    if (!post) {
      return res
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const slug = await generateUniqueSlug(req.body.title, req.body.id || null)
      ...
      if (req.body.id) {
        // Update
        if (!errors.isEmpty()) {
          const post = await PortfolioPost.findById(req.body.id).populate(
            'mediaIds'
          )
          const media = await Media.find().sort({ createdAt: -1 }).limit(50)
        ...
        const existingPost = await PortfolioPost.findById(req.body.id)
        Object.assign(existingPost, postData)
        await existingPost.save()
        req.session.flash = {
          type: 'success',
          message: 'Portfolio post updated successfully'
        }
        ...
        const newPost = new PortfolioPost(postData)
        await newPost.save()
        req.session.flash = {
          type: 'success',
      ...
      req.session.flash = {
        type: 'error',
        message: 'Error saving portfolio post'
      }
      res.redirect(
        req.body.id
          ? `/admin/portfolio/${req.body.id}/edit`
          : '/admin/portfolio/new'
      )
    }
  }
...
router.post('/:id/delete', async (req, res) => {
  try {
    ...
    req.session.flash = {
      type: 'success',
      message: 'Portfolio post deleted successfully'
    }
    res.redirect('/admin/portfolio')
  } catch (error) {
    ...
    req.session.flash = {
      type: 'error',
      message: 'Error deleting portfolio post'
    }
    res.redirect('/admin/portfolio')
  }
})
